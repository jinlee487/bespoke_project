...
$(function(){
	...
	$('#searchBtn').on("click",function(){
		self.location="/admin/popupCustomerList"
			+"?currpage=1"
	...
	$(".clickable-row").click(function() {
        opener.cidCallBack($(this).data("c_id"));
		window.close();
    }); //clickable-row_click
	...
	<main role="main" class="ml-sm-auto px-md-4">     
     <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
       ...
       <div class="btn-toolbar mb-2 mb-md-0">
		...
		<button type="button" id="searchBtn" class="btn btn-outline-primary form-inline mr-2"><i class="fa fa-search" >Search</i></button>
        <button type="button" id="cancelBtn" class="btn btn-outline-danger form-inline mr-2" onClick ="window.location='#';"><i class="fa fa-ban" ></i>인증 취소</button> 
       ...
       <table class="table table-striped table-sm" id="ctable" >
         <thead>
           <tr align="center" height=30>
             <th style="width:100px" scope="col">고객 번호</th><th style="width:50px" scope="col">그룹</th><th>성함</th>
             <th scope="col" style="text-align:left">연락처</th><th scope="col" style="text-align:left">주소</th><th scope="col">최종 수정일</th>
           </tr>
         </thead>
         <tbody>
			<c:forEach var="customerlist" items="${customerList}">
				<tr align="center" height=30 class='clickable-row' data-c_id='${customerlist.c_id}' onclick="window.location='#';">
	...
	<nav aria-label="Page navigation example">
	  <ul class="pagination justify-content-center">
			<c:if test="${pageMaker.prev}">
				<li class="page-item"><a class="page-link"  href="/admin/popupCustomerList${pageMaker.makeSearch(1)}">First</a></li>
				<li class="page-item"><a class="page-link"  href="/admin/popupCustomerList${pageMaker.makeSearch(pageMaker.sPageNo-1) }">prev</a></li>
		  	</c:if>
				<!-- 2) -->
			<c:forEach begin="${pageMaker.sPageNo}"  end="${pageMaker.ePageNo}" var="i">
				<c:choose>
					...
					<c:otherwise>
						<li class="page-item"><a class="page-link"  href="/admin/popupCustomerList${pageMaker.makeSearch(i)}">${i}</a></li>
					</c:otherwise>
				</c:choose>
			</c:forEach>
			<!-- 3) -->
		  <c:if test="${pageMaker.next && pageMaker.ePageNo > 0}">
				<li class="page-item"><a class="page-link"  href="/admin/popupCustomerList${pageMaker.makeSearch(pageMaker.ePageNo+1)}">next</a></li>
				<li class="page-item"><a class="page-link"  href="/admin/popupCustomerList${pageMaker.makeSearch(pageMaker.lastPageNo)}">Last</a></li>
			</c:if>
	  	</ul>
	  	<select class="form-inlinemb-3" name="perPageRow" id="perPageRow">
